...
import { createDummyPost } from '../utils/createDummyPost';
	...
	useEffect(() => {
		...
		if (q.where) {
			...
			if (Array.isArray(q.where)) {
				// Getting rid of first element to overwrite the query variable to be able to append more `where`s
				const { fieldPath, optionString, value } = q.where.shift()!;
				query = ref.where(fieldPath, optionString, value);
				q.where.forEach((w: queryType) => {
					const { fieldPath, optionString, value } = w;
					query = query.where(fieldPath, optionString, value);
				});
			} else {
